#include "UniformBufferProgram.hpp"
...
int UniformBufferProgram::run()
{
	...
	{
		static_cast<UniformBufferProgram*>(glfwGetWindowUserPointer(window))->
...
void UniformBufferProgram::framebuffer_size_callback(GLFWwindow* window, const int width, const int height)
{
	glViewport(0, 0, width, height);
}

void UniformBufferProgram::process_input(GLFWwindow* window)
{
...
void UniformBufferProgram::mouse_callback(GLFWwindow*, const double x, const double y)
{
	if (firstMouse)
	{
...
float UniformBufferProgram::matrix_minor(const float m[16], int r0, int r1, int r2, int c0, int c1, int c2)
{
	return
...
void UniformBufferProgram::matrix_cofactor(const float src[16], float dst[16])
{
...
unsigned int UniformBufferProgram::loadTexture(const std::string& path)
{
...
unsigned int UniformBufferProgram::loadCubemap(const std::vector<std::string>& pathsToTexturesFaces)
{
