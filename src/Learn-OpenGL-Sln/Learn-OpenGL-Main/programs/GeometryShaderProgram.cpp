#include "GeometryShaderProgram.hpp"
...
int GeometryShaderProgram::run()
{
	...
	//{
	//	static_cast<GeometryShaderProgram*>(glfwGetWindowUserPointer(window))->
...
void GeometryShaderProgram::framebuffer_size_callback(GLFWwindow* window, const int width, const int height)
{
	glViewport(0, 0, width, height);
}

void GeometryShaderProgram::process_input(GLFWwindow* window)
{
...
void GeometryShaderProgram::mouse_callback(GLFWwindow*, const double x, const double y)
{
	if (firstMouse)
	{
...
void GeometryShaderProgram::scroll_callback(GLFWwindow*, const double, const double yOffset)
{
	camera->processZoom(float(yOffset));
}

// The following code is from: https://github.com/graphitemaster/normals_revisited
float GeometryShaderProgram::matrix_minor(const float m[16], int r0, int r1, int r2, int c0, int c1, int c2)
{
	return
...
void GeometryShaderProgram::matrix_cofactor(const float src[16], float dst[16])
{
...
unsigned int GeometryShaderProgram::loadTexture(const std::string& path)
{
...
unsigned int GeometryShaderProgram::loadCubemap(const std::vector<std::string>& pathsToTexturesFaces)
{
