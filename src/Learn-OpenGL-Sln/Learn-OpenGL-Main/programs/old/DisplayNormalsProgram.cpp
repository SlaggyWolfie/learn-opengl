#include "DisplayNormalsProgram.hpp"
...
int DisplayNormalsProgram::run()
{
	...
	{
		static_cast<DisplayNormalsProgram*>(glfwGetWindowUserPointer(window))->
...
void DisplayNormalsProgram::framebuffer_size_callback(GLFWwindow* window, const int width, const int height)
{
	glViewport(0, 0, width, height);
}

void DisplayNormalsProgram::process_input(GLFWwindow* window)
{
...
void DisplayNormalsProgram::mouse_callback(GLFWwindow*, const double x, const double y)
{
	if (firstMouse)
	{
...
void DisplayNormalsProgram::scroll_callback(GLFWwindow*, const double, const double yOffset)
{
	camera->processZoom(float(yOffset));
}

// The following code is from: https://github.com/graphitemaster/normals_revisited
float DisplayNormalsProgram::matrix_minor(const float m[16], int r0, int r1, int r2, int c0, int c1, int c2)
{
	return
...
void DisplayNormalsProgram::matrix_cofactor(const float src[16], float dst[16])
{
...
unsigned int DisplayNormalsProgram::loadTexture(const std::string& path)
{
...
unsigned int DisplayNormalsProgram::loadCubemap(const std::vector<std::string>& pathsToTexturesFaces)
{
